...
## ADR-037 — M9 放置階段的直接量測：pcb1 幾何嚴重偏離，但這解釋不了 capsules

**狀態：Accepted**（2026-07-30）｜承接 [ADR-036](#adr-036) 的訊號，**改為先量測而非先訓練**

### 為什麼沒有直接開 v4 訓練實驗

ADR-036 的訊號指向「放置」，我原本要預註冊一個訓練實驗去驗它。但放置品質**可以直接量**，
不必先付訓練成本。先量測的結果**削弱了我自己原本要註冊的假說**——如果照原計畫走，
會花掉成本去驗一個用免費量測就能排除的方向。這與 [ADR-030](#adr-030)／
[ADR-031](#adr-031)／[ADR-034](#adr-034) 的模式一致：先量機制，再談實驗。

### 量測結果

| 物件 | 真實瑕疵元件中位數 | 合成放置中位數 | 比值 | 落在真實 p5–p95 內 | 合成最小值 > 真實中位數 |
|---|---:|---:|---:|---:|:---:|
| pcb1 | `911` px | `5,614` px | `6.16×` | **`42.7%`** | **是** |
| capsules | `2,619` px | `1,469` px | `0.56×` | **`100.0%`** | 否 |

實際套用的變換：pcb1 的 `affine.scale` 中位數 **`2.761`**，capsules **`0.671`**，
其中 capsules 有 `36.6%` 被縮到 0.5 倍以下。`configs/placement.yaml` 的
`transform.scale` 允許範圍是 `[0.2, 8.0]`。

### 三個由量測導出的結論

1. **pcb1 的放置幾何嚴重偏離真實分布**：過半合成面積落在真實 p5–p95 之外，
   且**連最小的合成放置都比真實瑕疵的中位數大**——偏離不是尾端少數樣本造成的。
   這是既有結果中**未被發現**的缺陷
2. **這無法解釋 capsules**：它的合成面積 `100%` 落在真實區間內，
   而它才是下游退步最嚴重的物件
3. 因此「放置幾何偏離分布」**不是所有物件共通的解釋**，
   任何以此為單一主因的說法都不成立

### 順帶排除的一條假說

我原先懷疑合成瑕疵被貼到膠囊之間的背景空隙。查程式後排除：`legal_roi()` 是
saturation 前景 ∩ DINOv2 結構區，而 M9 的驗證**已經強制**每個 mask 落在 legal ROI 內。
除非 ROI 定義本身有問題，這條路在建構上就不成立。

### 我在這次量測中犯的錯與修正

第一版把**真實 mask 底下**的像素拿去比**放置 mask 底下**的像素。這是無效的比較：
真實 mask 底下已經有瑕疵，放置 mask 底下還是乾淨背景，瑕疵本身就會改變飽和度與亮度，
所以差異是必然的，不能當成「落點不對」的證據。

修正為對兩邊都量 mask **外**寬 12 px 的環狀帶（兩邊都是無瑕疵的物件表面），並加 5 項測試
鎖住性質，其中一項直接斷言「環狀帶不得包含 mask 本身」。

修正後的數字：pcb1 周圍 intensity 由真實 `105.0` 對合成 `162.5`；
capsules `123.0` 對 `106.0`。**這組數字明確標記為描述性、不構成判定**——
本次沒有為「差多少算系統性不同」預先訂門檻，事後訂一個等於用結果決定標準。

### 防洩漏

983 個放置來源檔（pcb1 612、capsules 371）逐一比對 frozen test blocklist，
**命中 0**，並實作為 fail-closed 檢查而非事後聲明。診斷不重新生成、不訓練、不讀 test。

### 尚未預註冊的後續候選

- **pcb1 的 scale 上限**：`8.0` 顯然過寬，收緊是獨立於 capsules 問題的真實修正
- **capsules 的殘餘落差**：面積已排除、legal ROI containment 已由 M9 保證，
  剩下最可能的是**縫合**（M12 的羽化／Poisson）或 ROI 定義本身
...
## ADR-039 — v4 未能檢驗自己的假說；預註冊主判準選在已知無鑑別力的組合上

**狀態：Accepted**（2026-07-30）｜承接 [ADR-038](#adr-038)，**規則未作任何修改**

### 結果

| 物件 | real_only | db_current | db_inband | `D` | 指標可分辨 |
|---|---:|---:|---:|---:|:---:|
| capsules（對照） | `0.8133` | `0.6815` | `0.6889` | `+0.0074` | 是 |
| pcb1（主要） | `0.6944` | `0.6944` | `0.6944` | `+0.0000` | **否** |

**判定：`uninformative_primary_object`** —— 本次**不是**「面積限回真實分布沒有效果」，
而是**這個實驗沒有真正檢驗到自己的假說**。

Gate 第三次 `stopped`（`test_data_loaded = false`，未讀 test、未跑 3 seeds、未花錢）。

### 這是我的預註冊設計缺陷

[ADR-036](#adr-036) **已經明確記錄**「pcb1 上三個 candidate 的 Macro-F1 完全相同
（`0.6944`），指標在該物件沒有鑑別力」。ADR-038 卻仍然把
**pcb1 + Macro-F1** 訂為主要判準，等於用一個已知讀不出差異的組合去驗假說。

ADR-038 有預先寫入「無鑑別力的物件不得當作證據」，所以判定腳本正確地把結果標成
「未能檢驗」而不是「無效果」——**護欄有效，但設計本身不該讓護欄被觸發**。

### 對照組的設計是有效的

`capsules` 被預先指定為對照：它有 99.8% 的樣本本來就在區間內，因此不應明顯變動。
實測 `D = +0.0074`，遠低於 `0.05` 的異常門檻，**行為與預測一致**。
這代表 v4 的對照邏輯本身是好的，壞掉的是主要判準的指標選擇。

### 不得做的事（明確記錄，避免日後被誘惑）

pcb1 的 **AUROC 確實有鑑別力**（`real_only 0.9167`、`db_current 0.8833`、
`db_inband 0.8667`，`D_auroc = -0.0166`）。但 ADR-038 白紙黑字寫明主判準是
**Macro-F1**，事後改用 AUROC 就是換指標救結果，[ADR-027](#adr-027) 已明令禁止。
此處記錄該數值僅供將來設計參考，**不構成本次判定的任何部分**。

### 從三次 gate 全數未過學到的事

v2、v3、v4 的 confirmatory gate 連續三次未通過。這不是三次失敗的實驗，是三次**正確停止**：
每一次都在看到結果前寫死規則、看到結果後照規則停。合成資料在 classification 上
**仍然沒有正面的 test 證據**，而這個結論是三次獨立預註冊累積出來的，可信度高於單次結果。

### 給下一次預註冊的硬性要求

**任何預註冊在固定主判準之前，必須先確認該指標在該物件上有鑑別力。**
判準是既有資料上該指標對不同 candidate 是否曾經產生過不同的值。
這條與 [ADR-034](#adr-034) 的「結論必須由程式導出」屬於同一類：
**預註冊本身也需要被檢查，不能只檢查結果。**

---

## ADR-040 — v5 的**預註冊**：把 v4 複跑到 3 seeds，判定只用沒被看過的 seed 43／44

**狀態：Pre-registered**（2026-07-30）｜**執行前必須先 commit 本 ADR**

### 為什麼是複跑而不是換指標

[ADR-039](#adr-039) 指出 v4 的主判準（pcb1 + Macro-F1）已知無鑑別力。直覺的修法是換指標，
但那會踩到兩個問題：一是 pcb1 的 AUROC **已經在 ADR-039 被看過**（`-0.0166`），
二是 v2／v3／v4 **全部只有 seed 42 一個種子**，而本專案自己的
[ADR-032](#adr-032) 已經證明單 seed 的結論在這個資料規模下靠不住——
seed 42 的 AUPRO 提升就是那樣被推翻的。

因此 v5 的核心不是換指標，是**補 seed**，並用「判定只建立在沒被看過的資料上」
來處理指標汙染。

### 設計

沿用 v4 的三個 candidate 與**完全相同的兩臂樣本**：`configs/classifier_v5_base.yaml`
與 `classifier_v4_base.yaml` 經 diff 確認**位元相同**，因為 arm 成員由固定的
`SELECTION_SEED = 42` 決定，`--seed` 只改訓練期隨機性。這才使多 seed 成為**複跑**
而非重新抽樣。

| 項目 | 值 |
|---|---|
| Candidates | `real_only`、`db_current`、`db_inband` |
| 每物件合成張數 | `218`（兩臂相同，沿用 ADR-038） |
| Seeds | `42`（已存在，來自 v4）、**`43`、`44`（新跑）** |
| 新增 run | 3 × 2 × 2 = **12** |
| 成本 | 本機 RTX 4090，約 75 秒／run，約 15 分鐘。不需 Colab、不花錢 |

### 汙染揭露與判定範圍（本 ADR 的重點）

**已被看過的資料**：v4 的 seed 42 結果，包含 pcb1 的
`real_only 0.9167`、`db_current 0.8833`、`db_inband 0.8667`（AUROC，`D = -0.0166`）。

因此**判定只計入 seed 43 與 44**。seed 42 照常報告以保持完整，但
**不進入任何判定式**。這不是事後排除不利資料——排除規則在本 ADR 寫死，
且排除的是**唯一對我不盲的那個 seed**，方向對自己不利而非有利。

### 執行前就固定的判定規則（看到結果後不得更改）

主指標 **AUROC**（Macro-F1 在 pcb1 已知無鑑別力，見 ADR-039）。
主要物件 `pcb1`。對每個 seed 計算 `D = db_inband − db_current`，
取 **seed 43 與 44 的平均** `D̄`：

- `D̄ ≥ +0.02` → 判定**面積限回真實分布有幫助**
- `D̄ ≤ −0.02` → 判定**有害**
- 其餘 → 判定**無效果**

`0.02` **不是新訂的門檻**：它就是 [ADR-026](#adr-026) 早已固定的
per-object AUROC 容差，在此原值沿用。

**鑑別力檢查**（承 ADR-039 的硬性要求）：若在某個 seed 上三個 candidate 的 AUROC
完全相同，該 seed 標記為無鑑別力並排除；**若 43、44 兩個 seed 都無鑑別力，
判定為 `uninformative`**，不得報成「無效果」。

**對照組**：`capsules` 仍為對照（99.8% 樣本原本就在區間內）。
若其 `|D̄| ≥ 0.05`，標記為對照異常、本次結論可信度下降，
但**不得因此改寫 pcb1 的判定**。

### Confirmatory gate

沿用 ADR-026 的三個門檻**逐字不變**。**判定成立不等於 gate 通過**；
gate 未過就一律不得讀 frozen test。

### 不得發生的事

- 不讀 frozen test
- 不改超參、不改 100-step 預算、不改 `real_bad_share`、不改 218 這個張數
- **不改變兩臂的 sample id**（已由 base config 位元相同驗證）
- 不因結果不好而追加 seed 或改門檻
- seed 42 不得在事後被放回判定式

---

## ADR-041 — v5 判定：無效果，而且這次是**真正檢驗過**的無效果

**狀態：Accepted**（2026-07-30）｜承接 [ADR-040](#adr-040)，**規則未作任何修改**

### 結果

主指標 AUROC，主要物件 `pcb1`，`D = db_inband − db_current`：

| Seed | real_only | db_current | db_inband | `D` | 計入判定 |
|---:|---:|---:|---:|---:|:---:|
| 42 | `0.9167` | `0.8833` | `0.8667` | `-0.0167` | **否**（已被看過） |
| 43 | `0.9639` | `0.9611` | `0.9667` | `+0.0056` | 是 |
| 44 | `0.9694` | `0.9333` | `0.9361` | `+0.0028` | 是 |

`D̄ = +0.0042`，門檻 `±0.02` → **判定 `no_effect`**。
對照組 `capsules` `D̄ = +0.0046`，遠低於 `0.05` 的異常門檻，**行為正常**。
三個 seed 的 gate 全部 `stopped`，`test_data_loaded = false`。

### 與 v4 的關鍵差別

v4 判定為 `uninformative_primary_object`——**實驗沒有檢驗到自己的假說**。
v5 換成有鑑別力的指標後，三個 seed 上三個 candidate 的 AUROC 都不相同，
**實驗確實檢驗了假說，然後得到無效果**。

這兩者的證據價值完全不同：前者是設計失敗，後者是**有效的陰性結果**。

### seed 42 又一次是離群值

`pcb1` 的 `D` 在 seed 42 是 `-0.0167`，在 43／44 是 `+0.0056`／`+0.0028`——
**符號相反**。`capsules` 更明顯：seed 42 是 `-0.0509`，43／44 是 `+0.0000`／`+0.0093`。

若沿用單 seed 做法，本次會得到「面積限回真實分布**有害**」的相反結論。
這與 [ADR-033](#adr-033) 在分割上的發現完全一致：
**在這個資料規模下，單 seed 的方向不可信。**
排除 seed 42 的規則寫在執行前，而它剛好是唯一支持「有效應」的 seed——
排除的方向對結論不利，不是挑好看的資料。

### 這對整體結論的意義

`db_inband` 與 `db_current` 在三個 seed 上**都低於 `real_only`**。也就是說，
不論放置面積是否限回真實分布，**加入合成資料在 classification 上都比不加差**。
把面積修正到真實分布**不足以**救回合成資料——[ADR-037](#adr-037) 量到的 pcb1 幾何偏離
是真實缺陷，但**修掉它並不會讓合成資料變得有用**。

### 四次 gate 全數未過

v2、v3、v4、v5 連續四次 `stopped`，全部未讀 frozen test。這是四次**正確停止**，
每次都在看到結果前寫死規則。合成資料在 classification 上沒有正面 test 證據，
這個結論現在由**四次獨立預註冊**支撐，其中 v5 還帶三 seed 的複跑。

### 方法上的收穫

ADR-039 訂下「固定主判準前必須確認指標有鑑別力」，v5 是第一個遵守它的預註冊，
而且**立刻見效**：換成 AUROC 之後實驗才真正可讀。
另外新增一條可重複使用的做法——**當某批資料已被看過時，把判定範圍限制在未看過的部分**，
並在 ADR 中明確揭露被排除的是哪一批、方向為何。

---

## ADR-042 — 分割版 `--no-real-stats` 對照組：**決定不補**，改為把已量測的分類版揭露到 README

**狀態：Accepted**（2026-07-30）｜界定 [ADR-011](#adr-011) 的兌現範圍

### 背景

ADR-011 主動揭露程序化合成的洩漏面（mask 面積與長寬比被約束在 10 張 few-shot 訓練 mask
的 5–95 百分位內），並承諾「報表要同時列『用統計量』與『不用統計量』兩版的**分割**結果」。
分類版做了（`procedural_norealstats`），**分割版一直沒做**。

### 決定不補的理由

1. **分類版已經給出明確且方向一致的答案**，而且方向出乎意料——
   **不用統計量反而比較好**：pcb1 Macro-F1 `-0.0577`／AUROC `-0.0820`，
   capsules `-0.0125`／`-0.0119`（正值代表用統計量較好）。
   分割版回答的是「會不會推廣」，不是新問題
2. **架構代價高**：分割的證據鏈建立在「所有 run 都來自兩個經 CRC 與 validator 驗證的
   Colab ZIP」。本機補跑一組會破壞 `import_result_archive` 的單一封存不變量；
   要維持不變量就得再上一次 Colab 並重新打包
3. **連鎖修改廣**：8 個檔案的硬編計數（48/54 → 54/60）、README 重生、notebook 契約，
   估計數小時，而**完全不影響主結論**

### 真正沒被兌現的是揭露，不是實驗

ADR-011 的意圖是「讓讀者自己判斷那些統計量值多少」。實際狀況是**量測做了、但 README
完全沒提**——連 ADR-011 的正式措辭都沒有。這比缺一半實驗更傷。

因此本次的處置是：把分類版的量測結果寫成 README 的
`CLASSIFICATION_LEAKAGE_SURFACE` verified block（數字由 `verify_readme.py` 從
`classification.csv` 產生），並在正文說明洩漏面的範圍與方向。

### 記錄下來的結論

**這個被主動揭露的洩漏面，不只沒有帶來好處，還是負作用。**
主張「零真實瑕疵像素」的同時使用真實 mask 的聚合形狀統計量，在兩個物件上都讓下游更差。
