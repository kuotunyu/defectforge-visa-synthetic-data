...
- Search staged changes for `.env` content, API tokens, personal email addresses,
  `Co-Authored-By` trailers, or any author other than the repository owner.
- Never `git push`, tag, or upload to Hugging Face without explicit user approval;
  `scripts/verify_publish.py` passing is a precondition, not that approval.
